...
export interface PurchaseOrderForPrint {
  ...
  items?: { variant: { sku: string; product: { name: string } }; quantity: number; unitCost: number | string; totalCost: number | string }[]
}
...
export function printPurchaseOrder(po: PurchaseOrderForPrint, statusLabel: string): void {
  ...
  const html = `<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>أمر شراء ${po.poNumber}</title><style>
    *{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:13px;padding:24px;color:#000}
    ...
    <h2>رقم الأمر: <strong>${po.poNumber}</strong> | الحالة: ${statusLabel}</h2>
    <div class="grid">
